...
export const ValidatePassword: FC<ValidatePasswordProps> = ({ password }) => (
  <div className={'flex flex-col w-full text-[#008000]'}>
    <p className={getStyle(/^.{8,24}$/u.test(password))}>
      {/^.{8,24}$/u.test(password) ? <CheckIcon /> : <CloseIcon />} 8 e 24 caracteres
    </p>
    <p className={getStyle(/[A-Z]/u.test(password))}>
      {/[A-Z]/u.test(password) ? <CheckIcon /> : <CloseIcon />} 1 letra maiúscula
    </p>
    <p className={getStyle(/[a-z]/u.test(password))}>
      {/[a-z]/u.test(password) ? <CheckIcon /> : <CloseIcon />} 1 letra minúscula
    </p>
    <p className={getStyle(/[!@#$%^&*]/u.test(password))}>
      {/[!@#$%^&*]/u.test(password) ? <CheckIcon /> : <CloseIcon />} 1 carácter especial
    </p>
    <p className={getStyle(/[0-9]/u.test(password))}>
      {/[0-9]/u.test(password) ? <CheckIcon /> : <CloseIcon />} 1 número
    </p>
  </div>
);
